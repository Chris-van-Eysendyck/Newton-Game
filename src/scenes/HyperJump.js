...
export class HyperJump extends Phaser.Scene {
    ...
    create() {
        ...
        // Get the target level from registry (set by Start scene)
        const targetLevel = this.registry.get('targetLevel') || 'Level1';
        console.log('HyperJump: Going to', targetLevel);
        ...
        // After the jump effect, fade to target level
        this.time.delayedCall(2200, () => {
            this.cameras.main.fadeOut(500);
        });

        this.cameras.main.once('camerafadeoutcomplete', () => {
            console.log('HyperJump complete: Starting', targetLevel);
            this.scene.start(targetLevel);  // ← Now uses the target from registry!
        });
    }
