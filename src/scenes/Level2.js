...
export class Level2 extends Phaser.Scene {
    ...
    create() {
        ...
        this.numberButtons = [];  // For touch number pad
    ...
    setupKeyboardListeners() {
        ...
        this.input.keyboard.on('keydown', (event) => {
            if (this.mathProblemText && !this.levelText && this.inputEnabled) {
                ...
                // Exclude numpad keys as they're handled separately
                ...
                if (key === 'Backspace' || key === 'Delete') {
                    if (this.playerInput.length > 0) {
                        ...
                        console.log('Deleted last digit. Input now:', this.playerInput);
                    }
                }
            }
        });
    ...
    // Generate a random math problem (+ or -, result max 20)
    generateMathProblem() {
        const operators = ['+', '-'];
    ...
    trackGameStart() {
        // Simple fetch to count game starts - no personal data
        fetch('https://api.countapi.xyz/hit/newton-game/level2-starts')
            .then(res => res.json())
    ...
    trackLevelComplete() {
        // Simple fetch to count level completions - no personal data
        fetch('https://api.countapi.xyz/hit/newton-game/level2-completions')
            .then(res => res.json())
